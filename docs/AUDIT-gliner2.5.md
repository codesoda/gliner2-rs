...
| Every milestone gates | `cargo fmt`, `cargo clippy --all-targets -- -D warnings`, `cargo test` without AND with models | M0–M5 and direct ORT migration accepted; scoped tests/evidence in progress and evidence files. M6–M7 pending |
| Every milestone records | `PROGRESS-gliner2.5.md`: done/measured/next/questions; descriptive git commit; Astra review | Phase A, M0–M5 and direct ORT migration documented/committed/pushed (M5 at7508f03). Public explicit scoring is accepted with isolated full gates and ready for its scoped commit; later gates pending |
