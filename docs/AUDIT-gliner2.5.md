...
| M7 user docs | README "GLiNER2 vs 2.5: which to load", standalone consumer build/run via Git dependency | README/spec updated with actual publication; pushed-source sandboxed consumer execution pending |
| M7 latency | `RESULTS-gliner2.5.md`: measured CPU base v2/2.5 50/500/3000-word table, reproducible command/hardware | Pending |
| M7 CI/spec | CI skips absent models, checks real unit/committed-fixture coverage; specification §4/§5 updated to shipped state | Remote Python/MSRV/stable CI green at `974349a` (run35561052287); no-model gate reports 245 passing cases with48 explicit artifact skips, not inference coverage; final release revision must pass too |
...
| Every milestone gates | `cargo fmt`, `cargo clippy --all-targets -- -D warnings`, `cargo test` without AND with models | M0–M6, explicit scoring and ORT migration accepted. M7 corrected-artifact gates:245 strict tests/zero skips on both1.95 and1.91;245 no-model cases/48 skips; fmt/Clippy pass. Benchmark/consumer/release pending |
...
| Final GitHub state | clean committed branch, pushed SHA equality, CI result, documented installable revision | Implementation pushed with green CI through974349a; v0.2.0 prepared but no matching tag/release yet. Final documentation/benchmark/consumer acceptance still required |
