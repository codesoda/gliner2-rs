...
| Phase A goldens | ~30-case corpus design, >=3 long cases, edge/non-ASCII/CJK/tasks, tensors, tolerance | Designed in plan; generated and independently reproduced at M1 (30 cases, including all three long texts) |
...
| M5 records | real `RecordHead.forward_group` export, `decode_group` anchor/natural/legacy, all `extract_json*` APIs | Accepted: legacy JSON plus additive natural/latent/anchorless sidecar methods, ragged head orchestration, Hungarian/cardinality/exclusivity decode, source-faithful choice/validator formatting. `evidence/m5.json` and low-level preparation evidence |
| M5 gate | structure goldens and tutorial 3 on base 2.5 | Accepted: all5 structure cases exact formatted shape/text/order/UTF8 bounds, record confidence maximum3.6805868e-6 within original1e-3; parent tutorial3 release run,162 strict tests/zero skips on Rust1.95 and1.91,162 no-model tests,75 optimized tests, fmt/Clippy and fresh six-v2-tutorial gate pass |
| M6 relations | typed argument pair generator incl. caps, sparse biaffine scorer export, all `extract_relations*` APIs | Accepted: typed proposals, separate learned graph/runtime, source-faithful dedup and all relation APIs; numerical tie bug fixed without tolerance changes. Independent graph re-export and source probes; `evidence/m6.json` |
| M6 gate | relation goldens and tutorial 6 on base 2.5 | Accepted: four relation-only and two independent Unicode/mixed-task oracles exact on non-confidence outputs; maximum final confidence error8.940697e-7;217 strict tests/zero skips on1.95 and1.91, no-model/optimized/fmt/Clippy/v2 gates and release tutorial6 pass |
| M7 all bundles | `export_all_2_5.sh`; small/base/multi complete graphs + config/tokenizer + manifest | Passed all three independent seven-stage and 32-case native gates; exact non-confidence output, original tolerances. `evidence/m7-integration.json` |
| M7 manifest | architecture/version, upstream commit, HF revision, opset, ort; fp32 finite masks, no weights in git | Promoted after source/numerical/hash review; public Rust validator accepts all three, 16 authenticated files plus manifest each |
| M7 publication | upload tested bundles to `codesoda/gliner2-onnx`, verify remote download; ask if credentials missing | Published at `27310cd26099a387b9936a1e13b03d6a0700baf2`; 52 added files, no legacy graph changes; actual remote downloads verified |
| M7 downloaders | `scripts/download_models.py`, `examples/download_models.rs`; fresh complete bundle download | Both actual `all` downloads passed from empty destinations and isolated caches: identical 64-file SHA-256 inventories, 5,150,060,191 bytes each; parent independently rehashed both |
| M7 user docs | README "GLiNER2 vs 2.5: which to load", standalone consumer build/run via Git dependency | README/spec updated with actual publication; fresh pushed-source Rust1.91 consumer passes with Python-named execution denied; `evidence/m7-consumer.json` |
| M7 latency | `RESULTS-gliner2.5.md`: measured CPU base v2/2.5 50/500/3000-word table, reproducible command/hardware | Pending |
| M7 CI/spec | CI skips absent models, checks real unit/committed-fixture coverage; specification §4/§5 updated to shipped state | Remote Python/MSRV/stable CI green at `9332af6` (run35563422282); no-model gate reports 245 passing cases with48 explicit artifact skips, not inference coverage; final release revision must pass too |
...
| Explicit-span primitive | public `score_explicit_spans`, equivalence tests and example | Accepted: separate sparse-head API with strict UTF8 preflight and ordered duplicate-preserving results; independently regenerated three-case upstream oracle,178 strict tests/zero skips, no-model/Clippy/MSRV/v2 gates and release example. `evidence/public-explicit-spans.json` |
| Every milestone gates | `cargo fmt`, `cargo clippy --all-targets -- -D warnings`, `cargo test` without AND with models | M0–M6, explicit scoring and ORT migration accepted. M7 corrected-artifact gates:245 strict tests/zero skips on both1.95 and1.91;245 no-model cases/48 skips; fmt/Clippy pass. Consumer also passes; benchmark/release pending |
| Every milestone records | `PROGRESS-gliner2.5.md`: done/measured/next/questions; descriptive git commit; Astra review | M0–M6 and explicit scoring documented/committed/pushed (M6 at34321d1); M7 implementation pushed through974349a, publication/readback documented; M7 not yet fully accepted |
...
| Final GitHub state | clean committed branch, pushed SHA equality, CI result, documented installable revision | Implementation pushed with green CI and clean downstream-consumer proof through9332af6; v0.2.0 prepared but no matching tag/release yet. Final documentation/benchmark/release acceptance still required |
