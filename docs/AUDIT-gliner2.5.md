...
| Every milestone gates | `cargo fmt`, `cargo clippy --all-targets -- -D warnings`, `cargo test` without AND with models | M0–M6, public explicit scoring and direct ORT migration accepted; scoped tests/evidence in progress and evidence files. M7 pending |
| Every milestone records | `PROGRESS-gliner2.5.md`: done/measured/next/questions; descriptive git commit; Astra review | Phase A, M0–M5 and direct ORT migration documented/committed/pushed (M5 at7508f03). Public explicit scoring accepted/committed/pushed ate737cfb with isolated full gates; M6 reviewed/accepted and ready for its scoped commit; M7 pending |
