...
class ExtractorWrapper(torch.nn.Module):
    def __init__(self, span_rep: torch.nn.Module, count_pred: torch.nn.Module, count_embed: torch.nn.Module, max_width: int):
        super().__init__()
    ...
    def forward(self, text_emb: torch.Tensor, schema_emb: torch.Tensor, spans_idx: torch.Tensor):
        # text_emb: [L, H] (or [1, L, H])
        # schema_emb: [S, H] where S includes [P] + per-field markers
        ...
        # mask invalid spans (start/end == -1) to zero-safe indices
...
def main():
    ...
    wrapper = ExtractorWrapper(model.span_rep, model.count_pred, model.count_embed, model.max_width).to(device).eval()
    ...
    text_len = 8
    num_schema_tokens = 6  # [P] + five specials as a safe upper bound for dummy export
    num_spans = text_len * max_width

    dummy_text_emb = torch.randn((text_len, hidden), dtype=torch.float32, device=device)
    dummy_schema_emb = torch.randn((num_schema_tokens, hidden), dtype=torch.float32, device=device)
    dummy_spans_idx = torch.zeros((num_spans, 2), dtype=torch.long, device=device)

    out_path = os.path.join(OUT_DIR, "extractor.onnx")
    ...
    torch.onnx.export(
        ...
        dynamic_axes={
            ...
            "span_scores": {1: "num_schema_fields", 2: "text_len"},
        },
        opset_version=17,
    )

    print(f"Saved extractor ONNX to {out_path}")
    print(f"span_scores shape (dummy): [{max_count}, num_schema_fields, text_len, {max_width}]")
